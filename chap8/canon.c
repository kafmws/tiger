...
/* extract stms which could move ahead in expList(pointer list 'rlist')
   and restructure exps when necessary. */
static T_stm reorder(expRefList rlist) {
  if (!rlist)
    return T_Exp(T_Const(0)); /* nop */
  else if ((*rlist->head)->kind == T_CALL) {
    /* replace 'CALL' with 'ESEQ(Move(temp, CALL), temp)' to avoid
       later call's return value overwrite the former in eax */
    Temp_temp t = Temp_newtemp();
    *rlist->head = T_Eseq(T_Move(T_Temp(t), *rlist->head), T_Temp(t));
    return reorder(rlist);  // go on reordering the left part
  } else {
    struct stmExp hd = do_exp(*rlist->head);
    T_stm s = reorder(rlist->tail);
    if (commute(s, hd.e)) {  // safety to exchange the order of executing
      *rlist->head = hd.e;
      return seq(hd.s, s);  // retain hd.e, return SEQ(hd.s, s) to a front pos
    } else {
      Temp_temp t = Temp_newtemp();
      *rlist->head = T_Temp(t);
      // retain the value of 'hd.e', by 'Move(temp_t, hd.e)'
      // because excute 's' make an influence on 'hd.e', so make a copy before.
      // in fact move SEQ(hd.s, s) ahead,
      return seq(hd.s, seq(T_Move(T_Temp(t), hd.e), s));
    }
  }
}

// get reference list of args of callExp 'exp'
...
/* use 'reorder' extract the stms which could execute ahead.
   return (stm, exp) that stm will execute ahead exp in 'exp'.
   and execute (stm, exp) is equivalent to execute 'exp'. */
static struct stmExp do_exp(T_exp exp) {
  switch (exp->kind) {
    case T_BINOP:
  ...
  switch (stm->kind) {
    ...
    case T_MOVE:
      if (stm->u.MOVE.dst->kind == T_TEMP && stm->u.MOVE.src->kind == T_CALL)
        ...
        // MOVE(ESEQ(stm, exp), val_exp) -> SEQ(stm, MOVE(exp, val_exp))
...
static T_stmList linear(T_stm stm, T_stmList right) {
  if (stm->kind == T_SEQ)
    // linear 'SEQ(SEQ_, SEQList)' to 'SEQ(SEQ_.left, SEQ(SEQ_.right, SEQList))'
...
static C_stmListList mkBlocks(T_stmList stms, Temp_label done) {
  ...
  if (stms->head->kind != T_LABEL) {
    return mkBlocks(T_StmList(T_Label(Temp_newlabel("block")), stms), done);
  }
...
struct C_block C_basicBlocks(T_stmList stmList) {
  struct C_block b;
  b.label = Temp_newlabel("block_done");
...
static void trace(T_stmList list) {
  ...
  /* we want false label to follow CJUMP */
    ...
    if (false) {
      ...
      Temp_label false = Temp_newlabel("f");
